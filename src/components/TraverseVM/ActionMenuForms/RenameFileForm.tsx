...
export default function RenameFileForm(props: IRenameFileForm): JSX.Element {
    ...
    const { currentFileAttributes, setCurrentFileAttributes } = fileAttributesContext
    ...
    async function handleRenameFileClick(): Promise<void> {
        if (shouldConfirmSubmit.current) {
            confirmationMessage.current = "You are about to rename this file to an existing file named: "+filePathInputFieldRef.current+"\nAre you sure you want to overwrite this file?"
            ...
            try {
                ...
                try {
                    //console.log('About to send the following request: ', apiOptions)
                    await ApiRequest(apiOptions)
                    //props.setActiveFilePath(filePathInputFieldRef.current)
                    setCurrentFileAttributes({...currentFileAttributes, path: filePathInputFieldRef.current})
                    openSnackbar('Successfully renamed files', 'success')
                    handleClose()
                } catch (error) {
    ...
    return (
        <>
            <Dialog open={props.isRenameFileFormOpen} onClose={handleClose}>
